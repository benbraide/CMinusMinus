...
namespace cminus::evaluator{
	...
	protected:
		...
		bool evaluate_and_assign_(operators::id op, object::memory_ptr_type target, object::memory_ptr_type value) const{
			if (target->is_nan() || value->is_nan()){//Expression evaluates to NaN
				target->write(std::make_shared<memory::scalar_reference<target_type>>(target->get_type(), type::get_nan<target_type>::template value()));
				return true;
			}
			...
			case operators::id::compound_plus:
				target->write(std::make_shared<memory::scalar_reference<target_type>>(target->get_type(), (target->read_scalar<target_type>() + value->read_scalar<target_type>())));
				return true;
			...
			case operators::id::compound_divide:
				if ((computed_value = value->read_scalar<target_type>()) == static_cast<target_type>(0))
					...
					target->write(std::make_shared<memory::scalar_reference<target_type>>(target->get_type(), (target->read_scalar<target_type>() / computed_value)));
				return true;
			default:
				break;
			}
