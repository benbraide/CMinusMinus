...
namespace cminus::io{
	...
	public:
		...
		virtual std::size_t read(std::byte *buffer, std::size_t size) = 0;
		...
		template <typename target_type>
		std::size_t read_buffer(target_type *buffer, std::size_t count){
			return read((std::byte *)buffer, (sizeof(target_type) * count));
		}

		virtual std::size_t read_buffer(char *buffer, std::size_t count){
			return read(reinterpret_cast<std::byte *>(buffer), count);
		}

		virtual std::size_t read_buffer(wchar_t *buffer, std::size_t count){
	...
	protected:
		...
		virtual std::size_t read_scalar_(std::byte *buffer, std::size_t size, const std::type_info &type_info){
